...
function calcularTotales() {
    ...
    if (!impuestosHabilitados) {
        ...
        let textoImpuesto = nombreImpuesto || 'Impuesto';
        // Agregar detalle de valores
        if (ivaPorcentaje > 0 && ivaValorFijo > 0) {
            textoImpuesto += ` (${ivaPorcentaje}% + $${ivaValorFijo.toLocaleString('es-CO')})`;
        } else if (ivaPorcentaje > 0) {
            textoImpuesto += ` (${ivaPorcentaje}%)`;
        } else if (ivaValorFijo > 0) {
            textoImpuesto += ` ($${ivaValorFijo.toLocaleString('es-CO')})`;
        } else {
            textoImpuesto += ' (0%)';
        }

        document.getElementById('iva-porcentaje').textContent = textoImpuesto;
        document.getElementById('iva-porcentaje').classList.remove('text-muted');
    }
